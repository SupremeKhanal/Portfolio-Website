...
Mistakes in Questions: ${this.stats.mistakeList.join(", ") || "None"}`;
  ...
  template: `
  <div class="space-y-6">
    <div class="bg-slate-900/90 rounded-xl border border-slate-800 p-6 text-center space-y-3">
      ...
      <h1 class="text-4xl font-black text-slate-100">
        {{ stats.finalScore.toFixed(2) }} <span class="text-slate-500 text-2xl font-normal">/ {{ stats.totalPossibleMarks }}</span>
      </h1>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
      ...
      <div class="bg-slate-900/90 border border-slate-800 rounded-xl p-3.5 text-center">
        ...
        <div class="text-xs text-slate-400 mt-0.5">Guessed Answers</div>
      </div>
      <div class="bg-slate-900/90 border border-slate-800 rounded-xl p-3.5 text-center">
    ...
    <div v-if="showSubjects" class="bg-slate-900/90 rounded-xl border border-slate-800 p-5 space-y-4">
      <div class="flex items-center justify-between border-b border-slate-800 pb-3">
        ...
        <span class="text-[11px] text-slate-500 font-mono">Deduction Rate: {{ (params.negativeMarkingRate * 100).toFixed(0) }}%</span>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        <div v-for="(s, subj) in stats.subjectStats" :key="subj" class="bg-slate-950 border border-slate-800 p-3.5 rounded-lg space-y-2">
          <div class="flex justify-between items-center border-b border-slate-800/80 pb-1.5">
            ...
            <span class="text-[11px] font-mono text-slate-400">{{ s.score.toFixed(2) }} / {{ s.totalMarks }} Marks</span>
          </div>
    ...
    <div class="bg-slate-900/90 rounded-xl border border-slate-800 p-5 space-y-3">
      <div class="flex justify-between items-center border-b border-slate-800 pb-3">
        <h3 class="text-xs font-bold text-slate-300 uppercase tracking-wider">📊 Exam Performance & Guess Analysis</h3>
        <button @click="copySummary" class="bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs px-3 py-1.5 rounded-md font-semibold transition border border-slate-700">
      ...
      <div class="bg-slate-950 p-4 rounded-lg font-mono text-xs text-slate-300 leading-relaxed space-y-2 border border-slate-800">
        ...
        <div><strong class="text-slate-500">Unattempted Questions:</strong> {{ stats.unattemptedList.join(', ') || 'None' }}</div>
      </div>
    </div>
    ...
    <div class="space-y-5">
      ...
      <div v-for="(q, idx) in questions" :key="idx" class="bg-slate-900/90 rounded-xl border border-slate-800 p-5 space-y-3">
        ...
        <div class="pt-1 flex justify-end">
          <a :href="'https://www.google.com/search?q=' + encodeURIComponent(stripHtml(q.text))" target="_blank" class="text-slate-400 hover:text-slate-200 text-[11px]">🔍 Search question on Google ↗</a>
        </div>
      </div>
    </div>
  </div>
